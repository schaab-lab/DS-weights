...
Y = param.L;
w = 1;
%tau = (param.la1 + param.la2)/param.la2 .* param.theta * w * param.L .* [1, 0];
tau = param.theta * w * param.L .* [1, 1];

%% VFI
G.income = r * G.a + (1 - param.theta) * w .* param.zz + tau;
...
ss.V = V; ss.g = g; ss.c = hjb.c; ss.s = hjb.s; ss.A = hjb.A; ss.u = hjb.u; ss.dV = hjb.dV;
